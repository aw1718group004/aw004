...
def CalcTFIDF(con, D_ID, Table_ID, tablename): #Esta a dar erro
    '''
...
def GetDiShIn(dname1, dname2,con):
    '''
    Gets the ontology score based on Resnik measure, for each pair of detected diseases.
    '''
    sb_file = 'doid.db'
    curs = con.cursor()
    ...
    curs.execute('SELECT COUNT(*) FROM Ontologies WHERE Disease1="{}" and Disease2="{}" GROUP BY Disease1;'.format(dname1, dname2))
    count = curs.fetchall()
    ResDis = 0
    if len(count) == 0: #count so pode ser 0 ou 1
        ...
        curs.execute('INSERT INTO Ontologies (Disease1, Disease2, DiShIn) VALUES ("{}","{}", "{}" );'.format(dname1, dname2, str(ResDis)))
        return str(ResDis)
        
    else:
        curs.execute('SELECT DiShIn FROM Ontologies WHERE Disease1="{}" and Disease2="{}";'.format(dname1, dname2))
        ResDis = curs.fetchall()
        return ResDis[0][0]

def WriteTFPubMed(con, PMID, tf_all): 
    '''
    Update TF score for each article related to each disease.
    '''
    curs = con.cursor()
...
def WriteArticleDishin(con, A_ID):
    '''
    Update DiShIn score for each article related to each disease.
    '''
    import numpy as np
    menor=0
    curs = con.cursor()
    ...
    if len(dishins) != 0:
        if type(dishins[0]) == str:
            pass
        else: 
            menor = round(dishins[0],6)
            for i in dishins:
                if type(i) == str:
                    pass
                else:
...
def UpdateArticleDishin(con):
    ...
    curs = con.cursor()
    curs.execute("Select A_ID FROM PubMed WHERE A_ID > '27081093' ;")
    res = curs.fetchall()
    curs.execute('Select MAX(A_ID) FROM Articles WHERE DiShIn != NULL;')
    maxs = curs.fetchone()
    if maxs[0] == None:
        maxs = (0,)
    for AID in res:
...
def WriteTFFlickr(con, F_ID, tf_all):
    '''
    Update TF score for each picture related to each disease.
    '''
    curs = con.cursor()
    for disease in tf_all.keys():
        curs.execute('SELECT D_ID FROM Diseases WHERE Name="{}";'.format(disease))
...
def WriteFlickrDishin(con, F_ID, termcount):
    '''
    Update DiShIn score for each picture related to each related disease.
    '''
    import numpy as np
    curs = con.cursor()
    menor = 0
...
def UpdateFlickrDishin(con):
    ...
    res = curs.fetchall()
    curs.execute('Select MAX(F_ID) FROM Pictures WHERE DiShIn != NULL;')
    maxs = curs.fetchone()
    if maxs[0] == None:
        maxs = (0,)
    for FID in res:
...
def WriteTFTwitter(con, T_ID, tf_all):
    '''
    Update TF score for each tweet related to each disease.
    '''
    curs = con.cursor()
    for disease in tf_all.keys():
        #1st check if disease exists in Diseases
...
def UpdateTFTwitter(con):
    '''
    Update TF for each tweet related to each disease.
    '''
    curs = con.cursor()
...
def WriteTwitterDishin(con, T_ID, termcount):
    '''
    Update DiShIn score for each tweet related to each disease.
    '''
    import numpy as np
    curs = con.cursor()
    menor = 0
...
def UpdateTwitterDishin(con):
    ...
    res = curs.fetchall()
    curs.execute('Select MAX(T_ID) FROM Tweets WHERE DiShIn != NULL;')
    maxs = curs.fetchone()
    if maxs[0] == None:
        maxs = (0,)
    for TID in res:
...
def OrdDateArticles(con, diseaseID):
    '''
    Updates Date relevance for Articles: the last picture, by disease, entering the database will get an higher score.
    '''
    curs = con.cursor()
...
def OrdDateAllArticles(con):
    '''
    Sort articles by ID to update date relevance.
    '''
    curs = con.cursor()
...
def OrdDateTweets(con, diseaseID):
    '''
    Updates Date relevance for Tweets: the last picture, by disease, entering the database will get an higher score.
    '''
    curs = con.cursor()
...
def OrdDateAllTweets(con):
    '''
    Sort tweets by ID to update date relevance.
    '''
    curs = con.cursor()
    curs.execute('SELECT D_ID from Diseases;')
    d_ids = curs.fetchall()
...
def OrdDatePictures(con, diseaseID):
    '''
    Updates Date relevance for Pictures: the last picture, by disease, entering the database will get an higher score.
    '''
    curs = con.cursor()
...
def OrdDateAllPictures(con):
    '''
    Sort pictures by ID to update date relevance.
    '''
    curs = con.cursor()
    curs.execute('SELECT D_ID from Diseases;')
    d_ids = curs.fetchall()
